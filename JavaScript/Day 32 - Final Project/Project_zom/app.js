...
function createCard(restaurant) {
    ...
    cardHeader.append(name, rating);

    // Card footer
    const cardFooter = document.createElement('div');
    cardFooter.classList.add('card-footer');
    const foodType = document.createElement('span');
    foodType.textContent = restaurant.food_type;
    ...
    // Append all sections to card content
    cardContent.append(cardHeader, cardFooter, cardLocation);
...
function renderRestaurants(restaurants) {
    ...
    root.replaceChildren(); // Clear previous content
...
function applySorting(option, restaurants) {
    switch (option) {
        case 'rating':
...
document.getElementById('Alcohol').addEventListener('click', () => {
    const filteredRestaurants = restaurants.filter(r => r.alcohol);
    renderRestaurants(filteredRestaurants);
});

document.getElementById('Rating').addEventListener('click', () => {
    const filteredRestaurants = restaurants.filter(r => r.rating > 4.5);
    renderRestaurants(filteredRestaurants);
});
...
document.getElementById('applyFilter').addEventListener('click', () => {
    ...
    const sortedRestaurants = applySorting(selectedOption, [...restaurants]); // Avoid mutating the original array
